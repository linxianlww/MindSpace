...
class ShareService {
  ...
  Future<void> shareText(String text, {String? subject}) {
    return Share.share(text, subject: subject ?? 'MindSpace');
  }
  ...
  Future<void> shareFiles(List<String> paths, {String? text}) async {
    ...
    await Share.shareXFiles(files, subject: 'MindSpace', text: text);
  }

  /// 把截图得到的字节先写入临时目录再以图片分享。
  Future<void> shareBytes(Uint8List bytes,
      {String fileName = 'mindspace.png'}) async {
